...
func NewRoutingHandler(
    ...
    return &RoutingHandler{identities, members, pub, catchall}
}
...
func (h *RoutingHandler) Resolve(c *gin.Context) {
    ...
    // 4. Get members (emails only)
    members, err := h.Members.ListMembersForIdentity(c, identity.ID)
    if err != nil {
        c.JSON(500, gin.H{"error": "failed to list members"})
        return
    }
