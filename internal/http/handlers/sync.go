...
func (h *SyncHandler) UploadSyncKeys(c *gin.Context) {
	...
	// Get email of identity and compute hash
	Identity, err := h.Identities.GetById(c, identityUUID)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "identity not found"})
		return
	}
	email := Identity.Email
	wkdHash := wkd.WKDHash(email)
	// 1. Sauvegarder la clé publique transmise par le SDK
	_, err = h.PublicKeys.InsertPublicKey(c, identityUUID, req.ArmoredPublicKey, wkdHash, true)
	if err != nil {
